...
[t_m,t_n] = size(spikes.info.tree);
if t_n == 2
    spikes.info.tree = [spikes.info.tree; to from];  % add entry to tree structure
elseif t_n == 4
    spikes.info.tree = [spikes.info.tree; to from 0 0];  % add entry to tree structure
else
    error('Dunno what''s going on with the info tree shape')
end
