...
%
   ...
   % get data
    ...
    % bin data
    tlims = [min(spikes.spiketimes) max(spikes.spiketimes)];% [0 sum(spikes.info.detect.dur)];  
    ...
    % show histogram
    hold on
    bar(edges(1:end-1) ,vals,1.0);  shading flat; % '+ mean(edges(1:2))' was at column 24 on this line 
    hold off
