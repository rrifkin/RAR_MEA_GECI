function splitmerge_tool( spikes, show_clusters, h, varname, savefilename)
...
%
    ...
    if nargin < 2 || isempty(show_clusters) || isequal( show_clusters,'all') 
    ...
    if nargin < 4 || isempty(varname), varname = 'spikes'; end
    if nargin < 5, savefilename = 'spikes.mat'; end
    ...
    figdata.default_output_name = savefilename;
...
function saveSpikesToFile(varargin)
    ...
    if isempty(filename_default)
        filename = figdata.default_output_name;
    else
...
function makePlotFeatures(varargin)
      ...
      selected = figdata.selected;
      %{
      %% My alteration to allow for my rotating version instead. Ed 24/01/2015 23:01:19
      figure
      if isempty(selected), selected = 'all'; end
      plot_features(figdata.spikes,selected)
      %}
      ss_pca_plot(figdata.spikes,'units',selected);
end

% show the all features figure (Ed addition) (executes on press of 'y')
function full_pca(varargin)
    figdata = get(varargin{end},'UserData');
    selected = figdata.selected;
    ss_full_pca_plot(figdata.spikes,selected);
end

% recalculates features for just selected waves and shows plot (Ed addition)
% (executes on press of 'n')
function sub_pca(varargin)
    figdata = get(varargin{end},'UserData');
    selected = figdata.selected;
    ss_sub_pca_plot(figdata.spikes,selected);
end

% plots PCA scores 1 and 2 with a time slider so can see how PC varies
% through time (Ed addition - executes on press of 't')
function time_pca(varargin)
    figdata = get(varargin{end},'UserData');
    pca_time_slider(figdata.spikes);
end
