...
if isempty(findobj(h,'Tag','xslider'))
    ...
    set(h,'WindowScrollWheelFcn', {@scrolling_callback,h});
...
% if current object is a slider, only then call the slider callback
    ...
    function scrolling_callback(src,callbackdata,varargin)
        h = varargin{end};
        is_shift = get(h,'selectiontype');
        my_objects = get_scalable_objects( h);
        % if there are any axes or uipanels
        if ~isempty(my_objects )
            if strcmp(is_shift,'extend')
                slider = findobj(h,'Tag','xslider');
            else
                slider = findobj(h,'Tag','yslider');
            end
            set( [my_objects h slider], 'Units','pixels')
            % get some numbers
            margin  = get(slider,'userdata');
            if callbackdata.VerticalScrollCount > 0
                margin = margin - 10;
            else
                margin = margin + 10;
            end
            set(slider,'userdata',margin);
            slider_callback(h);
        end
    end
