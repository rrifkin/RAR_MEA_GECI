...
%
    ...
    spikes.params.max_method = 'raw';
    spikes.params.thresh = 4;            % for 'auto', set number of rqq deviations above background noise
 %  spikes.params.thresh =    [-5.3897 -5.0999 -5.1816 -5.5274]; % for 'manual', set specific thresholds for each channel
    ...
    if strcmp(spikes.params.max_method,'auto')
        spikes.params.maxThresh = 50;       % maximum multiple of rqq to drop over (goes +ve and -ve)
        spikes.params.rawMaxThresh = [-800 800];   % will go to whichever is larger out of these 2
    else
        spikes.params.maxThresh = 1000;
    end
    % sorting parameters
    ...
    % Another Ed addition:
    spikes.params.dimension_reduction = 0;  % whether to apply dimensionality reduction (t-SNE as per L.J.P. van der Maaten)
    spikes.params.perplexity = 50;
    spikes.params.theta = 0.4;
    spikes.params.initial_dims = 10;
    spikes.params.output_dims = 3;
    %%
    % DISPLAY PARAMETERS
    %
    ...
    spikes.params.display.label_categories = {'in process', 'good unit', 'multi-unit', 'garbage', 'needs outlier removal','high % missing'};
    spikes.params.display.label_colors = [ .7 .7 .7; .3 .8 .3; .3 .3 .8;  .8 .3 .3; .7 .7 .3; 0.086275 0.32157 0.51765];
    
    % figure layout
    spikes.params.display.default_figure_size = [.05 .1 .9 .8];  % location of default figure
    % Addition to run on saturn when local machine has 2 screens being
    % registered as one:
    [~,hostname] = system('hostname');
    if strcmp(deblank(hostname),'saturn2')
        temp = get(0,'screensize');
        if temp(3) > 1920
            spikes.params.display.default_figure_size = [.05 .1 .4 .8];
        end
    end
    % </end saturn dual screen addition>
