function normalized_PC_bins = RAR_rapidsort_cross_correl_normalized (processed_spikes_file, epoch, window, bin_width)

	% epoch, window and bin_width are in seconds
	...
	mean_PC_firing_rate_per_channel = mean_PC_firing_rate / channels;

	normalized_PC_bins = (PC_bins / mean_PC_firing_rate_per_channel) / IN_num;

	output_file = strcat(processed_spikes_file(1:end-20), '_window_', num2str(window), '_bin_width_', num2str(bin_width), '_IN-triggered_PC_bins_normalized.csv');
