...
function RAR_plot_rapidsort_excerpt_concat_nosmoothing (DMSO_firing_rates_file, GiGA1_firing_rates_file, timepoints, seconds_per_inch, PC_Hz_per_inch)
	...
	PCfr_max = max(rapidPCfr);
	PC_y_maximum = PCfr_max + 20;
	...
	plot(rapidPCt(indexPC(:)), rapidPCfr(indexPC(:)), 'Color', 'blue', 'LineWidth', line_width);
	...
	plot(rapidINt(indexIN(:)), rapidINfr(indexIN(:)), 'Color', 'red', 'LineWidth', line_width);

	hold (ax, 'off');

	output_file = strcat (DMSO_firing_rates_file(1:end-4), "DMSO_GiGA1_concatenated_firing_rates_nosmoothing.pdf");
