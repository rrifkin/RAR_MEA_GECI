function whole_slice_normalized_PC_bins = RAR_rapidsort_cross_correl_per_channel (processed_spikes_file, epoch)
    ...
    for p = 1:length(spikes)
        if ~isempty(spikes(p).waveforms)
			...
			% excludes channels with no activity
			if any(normalized_PC_bins)
	...
	whole_slice_normalized_PC_bins = mean(per_channel_normalized_PC_bins,1);

	output_file = strcat(processed_spikes_file(1:end-20), 'IN-triggered_PC_bins_per_channel.csv');
	writematrix(per_channel_normalized_PC_bins, output_file);

	output_file = strcat(processed_spikes_file(1:end-20), 'IN-triggered_PC_bins_per_channel_whole_slice.csv');
