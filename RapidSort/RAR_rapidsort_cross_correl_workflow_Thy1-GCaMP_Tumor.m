function RAR_rapidsort_cross_correl_workflow_Thy1-GCaMP_Tumor(epoch, window, bin_width)
	...
	for i = 1:numel(experiments)
		...
		GiGA1_normalized_PC_bins = RAR_rapidsort_cross_correl (GiGA1_processed_spikes_file, epoch, window, bin_width);
		all_GiGA1_bins = [all_GiGA1_bins; GiGA1_normalized_PC_bins];

	end

	cd ~;
	DMSO_output_file = strcat('Thy1-GCaMP_Tumor_DMSO_IN-triggered_PC_bins_window_', num2str(window), '_bin_width_', num2str(bin_width), '.csv');
	writematrix(all_DMSO_bins, DMSO_output_file);
	GiGA1_output_file = strcat('Thy1-GCaMP_Tumor_GiGA1_IN-triggered_PC_bins_window_', num2str(window), '_bin_width_', num2str(bin_width), '.csv');
