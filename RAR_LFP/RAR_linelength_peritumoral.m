function RAR_linelength_peritumoral (LFP_file, artifact_file, bad_channels_file, offslice_channels_file, inactive_channels_file, peritumoral_file, excluded_minutes)

    % Parameters
    sample_rate = 2000; % in Hz
    output_suffix = '_linelength_peritumoral_excluding_min';
	...
	mean_peri_linelength = mean(peri_linelength);

	non_linelength = sum(abs(diff(nonperitumoral_data,[],2)'))./size(nonperitumoral_data,2);
    non_linelength = non_linelength * sample_rate;
	mean_non_linelength = mean(non_linelength);

    % Output data to .csv file
	output_array = ["mean peritumoral linelength per second per channel", mean_peri_linelength;
	"mean non-peritumoral linelength per second per channel", mean_non_linelength];
	output_file = strcat(LFP_file(1:end-16), output_suffix, excluded_min_string, '.csv');
