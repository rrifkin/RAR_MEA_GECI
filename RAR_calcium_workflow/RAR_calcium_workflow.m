...
function RAR_calcium_workflow (radius, eleclocs_file, ACSF_image_directory, ZMG_image_directory)
	...
	ZMG_filename_prefix = erase(ZMG_list_of_tifs(1,1).name,".tif"); % extract filename prefix from first element of list of tifs
	ZMG_output_filename = strcat(ZMG_filename_prefix, ".mat"); 
	RAR_caproc_parloop (radius, eleclocs_file, ZMG_number_of_tifs, ZMG_filename_prefix, ".tif", ZMG_last_tif_frames, ZMG_output_filename) ; 
