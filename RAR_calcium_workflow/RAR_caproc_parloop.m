...
function output_filename = RAR_caproc_parloop (eleclocs_file, list_of_tifs)
	...
	output_filename = strcat(current_path, '/', tif_filename_prefix, '_elec_intensities.mat');
	radius_filename = strcat(current_path, '/', tif_filename_prefix, '_radius.mat');
	...
	elec_intensities = cell(1, number_of_tifs);
	...
	save(output_filename, 'elec_intensities', '-v7.3');

	% save radius to mat file
	save(radius_filename, 'radius', '-v7.3');
end
